...
我々の新しい手法は透過境界面を通るサブパスを生成します。概念的に、許容パス$\bf{X}$をシードパス$\bf{Y}$を説どうすることでサンプルし、マルコフ連鎖の分野外でこれを行います。
スタンダードなモンテカルロの分野にいるとして、$\bf{X}$の確率密度は

$$p(\bf{X}) = \int_{\bf{Y} \in \Omega} p(\bf{X|Y}) \cdot p(\bf{Y}) d\bf{Y} \tag{5}$$

と表されます。この積分はかなり高コストです。
MLTは見事にこの積分を平衡分布が正しいことを確認することで回避しています。
これを達成する一つの方法としては受け入れ確立を現在のサンプルと推定したサンプルにのみ依存させることがあります。

### COmputing the Marginalised Distribution
モンテカルロの分野では、我々は僅かな条件を導入することで達成することができました。
まず、シードパス$Y$が常に現在のシェーディング店$x_b$と光源点$x_c$によって決定的に定義されるシングルステップの摂動を行います。シードパスは許容輸送パスの構築し始めるのに独立して使う時、エネルギーバランスは必要でありません。
さらに、我々はハーフベクトル$H_{b+1,c-1}$のセットが与えられ、許容パス$X$とシードパス$Y$が全単射である事と仮定します。
これの意味はEq.5の積分が$p(X|Y) = \delta(\bf{Y})$と$Y$となるため０ではなくなります。これで周辺分布の評価をすることができます。

アンバイアスパス空間レンダラーで我々のテクニックを実装するために、サンプリング手順とPDF評価をMISフレームワークで行う必要がある。このセクションの残りはハイレベルなサンプリングとPDF計算の概要となります。具体的な話は次にセクションにて展開していきます。
